...
class AdminIndexController extends AdminBaseController
{
    ...
    function del(){
        ...
        //删除付费内容
        Db::name("paidprogram")->where("uid={$id}")->delete();
        //删除付费内容评论
    ...
    function setrecommend(){
        ...
        $rs = DB::name('user')->where("id={$id}")->setField('isrecommend',$isrecommend);
        if(!$rs){
            $this->error("操作失败！");
        }
        DB::name("live")->where(array("uid"=>$id))->setField('isrecommend',$isrecommend);
        if($isrecommend==1){
            $action="设置推荐会员：{$id}";
        }else{
            $action="取消推荐会员：{$id}";
