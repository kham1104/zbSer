...
class Api_Shop extends PhalApi_Api {
	...
	*/
	...
	public function shopApply(){
		...
		if($receiver){
			if(mb_strlen($receiver)>4){
	        	$rs['code']=1001;
	        	$rs['msg']="收货人姓名长度不能超过4个字";
	        	return $rs;
	        }
		}else{
			$receiver=$username;
	 ...
	 * @return string info[0].shopinfo.uid 店铺用户ID
	 * @return string info[0].shopinfo.user_nicename 店铺用户昵称
	 * @return string info[0].shopinfo.sale_nums 店铺总销量
	 * @return string info[0].shopinfo.quality_points 店铺商品质量评分
	 * @return string info[0].shopinfo.service_points 店铺服务质量评分
	 * @return string info[0].shopinfo.express_points 店铺物流速度评分
	 * @return string info[0].shopinfo.certificate 店铺营业执照
	 * @return string info[0].shopinfo.other 店铺其他证件
	 * @return string info[0].shopinfo.service_phone 店铺客服热线
	 * @return string info[0].shopinfo.province 店铺省份
	 * @return string info[0].shopinfo.city 店铺城市
	 * @return string info[0].shopinfo.area 店铺所在地区
	 * @return string info[0].shopinfo.name 店铺名称
	 * @return array info[0].shopinfo.avatar 商品封面
	 * @return array info[0].shopinfo.composite_points 店铺综合评分
	 * @return array info[0].shopinfo.goods_nums 店铺在售商品数量
	 * @return array info[0].shopinfo.address_format 店铺地址格式化
