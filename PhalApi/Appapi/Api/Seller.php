...
class Api_Seller extends PhalApi_Api {
	...
	public function getRules() {
		return array(
            ...
            'setExpressInfo'=>array(
                ...
                'express_number' => array('name' => 'express_number', 'type' => 'int',  'desc' => '物流单号'),
                'time' => array('name' => 'time', 'type' => 'string', 'desc' => '时间戳'),
                'sign' => array('name' => 'sign', 'type' => 'string', 'desc' => '签名字符串'),
            ),
	...
	public function setExpressInfo(){
		...
		if($uid<0||$token==''||!$orderid||!$expressid||$express_number==''||!$time||!$sign){
			$rs['code']=1001;
			$rs['msg']='参数错误';
			return $rs;
		}
    ...
    public function getGoodsOrderInfo(){
        ...
        if($order_info['status']==1){ //订单待发货
        	...
        	$order_info['status_desc']='请尽快发货,逾期将会自动关闭订单,钱款会退还给买家,多次逾期可能会影响电商功能使用';
        }
        ...
        $rs['info'][0]['shop_info']=$model->getShop($order_info['shop_uid']);
