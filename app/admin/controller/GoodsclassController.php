...
class GoodsclassController extends AdminbaseController {
    ...
    function index(){
        ...
        $lists->each(function($v,$k){
            ...
            //获取二级分类
            $two_list=Db::name("shop_goods_class")->field("gc_id,gc_name,gc_isshow,gc_sort,gc_addtime,gc_edittime")->where("gc_parentid={$v['gc_id']}")->order("gc_sort")->select()->toArray();
            if($two_list){
                foreach ($two_list as $k1 => $v1) {
                    //查询三级分类
                    $three_list=Db::name("shop_goods_class")->field("gc_id,gc_name,gc_isshow,gc_sort,gc_addtime,gc_edittime")->where("gc_parentid={$v1['gc_id']}")->order("gc_sort")->select()->toArray();
   ...
   protected function getAdminClassList(){
        ...
        //获取一级分类
        $lists=Db::name("shop_goods_class")->field("gc_id,gc_name")->where("gc_parentid=0")->order("gc_sort")->select()->toArray();
        ...
        foreach ($lists as $k => $v) {
            ...
            $two_list=Db::name("shop_goods_class")->field("gc_id,gc_name")->where("gc_parentid={$v['gc_id']}")->order("gc_sort")->select()->toArray();
