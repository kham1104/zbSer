...
class Model_Seller extends PhalApi_Model_NotORM {
    ...
    public function setGoods($data){
        ...
        //向商品表写入数据
        DI()->notorm->shop_goods->insert($data);
    }
    // 获取不同分类下的商品总数
    public function getGoodsNums($uid){
        ...
        $onexamine=DI()->notorm->shop_goods->where("uid={$uid} and status=0")->count();
        if($onexamine){
            $res['onexamine']=$onexamine;
        }

        $remove_shelves=DI()->notorm->shop_goods->where("uid={$uid} and status!=0 and status!=1")->count();
        if($remove_shelves){
    ...
    public function getGoodsList($uid,$type,$p){
        ...
        switch ($type) {
            ...
            case 'remove_shelves':
                $where="uid={$uid} and status!=0 and status!=1";
    ...
    public function upGoodsSpecs($uid,$goodsid,$specs){
        ...
        $rs=DI()->notorm->shop_goods->where("id=?",$goodsid)->update(array('specs'=>$specs));
        if($rs===false){
            return -1;
        }
